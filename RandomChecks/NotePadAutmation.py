...
app = Application().start('notepad.exe', timeout=50)

# Main window specification
main_dlg = app.UntitledNotepad
main_dlg.wait('visible')

# Print all controls on the dialogt
# main_dlg.print_control_identifiers()

main_dlg.Edit.type_keys("Type something now and save as PDF automatically",
                        with_spaces=True,
                        with_newlines=True,
                        pause=0.5,
                        with_tabs=True)
...
save_dlg.wait('ready')
file_path = 'C:\Temp\VSCodeProjects\Practice_project'
